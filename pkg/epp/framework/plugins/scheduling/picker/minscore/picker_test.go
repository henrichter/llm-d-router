...
func TestPickMinScorePicker(t *testing.T) {
	...
	tests := []struct {
		...
		tieBreakCandidates int // tie break is random, specify how many candidate with min score
	}{
		{
			name:   "Single min score",
			picker: NewMinScorePicker(1),
			input: []*fwksched.ScoredEndpoint{
			...
			output: []fwksched.Endpoint{
				&fwksched.ScoredEndpoint{Endpoint: endpoint1, Score: 10},
			},
		},
		{
			name:   "Multiple min scores, all are equally scored",
			picker: NewMinScorePicker(2),
			input: []*fwksched.ScoredEndpoint{
				{Endpoint: endpoint1, Score: 30},
				{Endpoint: endpoint2, Score: 30},
				{Endpoint: endpoint3, Score: 50},
			},
			output: []fwksched.Endpoint{
				&fwksched.ScoredEndpoint{Endpoint: endpoint1, Score: 30},
				&fwksched.ScoredEndpoint{Endpoint: endpoint2, Score: 30},
			},
			tieBreakCandidates: 2,
		},
		{
			name:   "Multiple results sorted by lowest score, more pods than needed",
			picker: NewMinScorePicker(2),
			input: []*fwksched.ScoredEndpoint{
				{Endpoint: endpoint1, Score: 30},
				{Endpoint: endpoint2, Score: 25},
			...
			output: []fwksched.Endpoint{
				&fwksched.ScoredEndpoint{Endpoint: endpoint3, Score: 20},
				&fwksched.ScoredEndpoint{Endpoint: endpoint2, Score: 25},
			},
		},
		{
			name:   "Multiple results sorted by lowest score, less pods than needed",
			picker: NewMinScorePicker(4), // picker is required to return 4 pods at most, but we have only 3.
			input: []*fwksched.ScoredEndpoint{
				{Endpoint: endpoint1, Score: 30},
				{Endpoint: endpoint2, Score: 25},
				{Endpoint: endpoint3, Score: 20},
		...
		{
			name:   "Multiple results sorted by lowest score, num of pods exactly needed",
			picker: NewMinScorePicker(3), // picker is required to return 3 pods at most, we have only 3.
			input: []*fwksched.ScoredEndpoint{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.tieBreakCandidates > 0 {
				testMinScoredEndpoints := test.output[:test.tieBreakCandidates]
				gotMinScoredEndpoints := got[:test.tieBreakCandidates]
				diff := cmp.Diff(testMinScoredEndpoints, gotMinScoredEndpoints, cmpopts.SortSlices(func(a, b fwksched.Endpoint) bool {
